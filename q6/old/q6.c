...
#define S 3
#define Q 2
>>>>>>> bccc5fba1c4feba1df54b34fdc7c34778e61bc28
...
=======
pthread_mutex_t reallocationBufferMutex = PTHREAD_MUTEX_INITIALIZER;

pthread_cond_t doMaintenceCond[S] = { PTHREAD_COND_INITIALIZER };
pthread_cond_t maintenceCompleteCond = PTHREAD_COND_INITIALIZER;

pthread_barrier_t maintenceCompleteBarrier, finalMaintence, finalMaintenceMessage;

int xrayedLuggageCount = 0;
int blockedSensors = 0;
int sensorToReallocate[S] = { 0 }; //0 means it doesn't need reallocation, 1 means it does
int reallocationBuffer = 0;
int doFinalmaintence = 0;
>>>>>>> bccc5fba1c4feba1df54b34fdc7c34778e61bc28
...
void lockAllLines(){ //lock all lines but the one indicated by "except"
    int i;
    for(i = 0; i < N; i++){
...
void unlockAllLines(){// unlock all lines but the one indicated by "except"
    int i;
    for(i = N-1; i >= 0; i--){
...
void lockAllSensors(){ //lock all sensors but the one indicated by "except"
    int i;
    for(i = 0; i < S; i++){
...
void unlockAllSensors(){ //unlock all sensors but the one indicated by "except"
    int i;
    for(i = S-1; i >= 0; i--){
...
void * xrayLuggage(void * s){
    ...
    while (doFinalmaintence == 0){
        ...
        printf("sensor %d acquired its lock\n", this);
        printf("sensor: %d, count: %d, queue: %d, blocked: %d, total count: %d\n", this, sensor[this].count, sensor[this].queue, sensor[this].blocked, xrayedLuggageCount);
        if(sensor[this].count < Q){ //if there's no need for maintence
            if(sensor[this].queue > 0){
                sensor[this].queue--;
                sensor[this].count++;
                pthread_mutex_lock(&xrayedLuggageMutex);
            ...
            sensor[this].blocked = 1;
            pthread_mutex_lock(&reallocationBufferMutex);
            reallocationBuffer += sensor[this].queue;
            sensor[this].queue = 0;
            pthread_mutex_unlock(&reallocationBufferMutex);
            pthread_mutex_lock(&blockedSensorsMutex);
            blockedSensors++;
            pthread_mutex_unlock(&blockedSensorsMutex);
            //printf("sensor %d blocked. count: %d, queue: %d, total count: %d, reallocation buffer: %d\n", this, sensor[this].count, sensor[this].queue, xrayedLuggageCount, reallocationBuffer);
            while (sensor[this].blocked) {
                //printf("sensor %d waiting for maintence, state: %d. blockedSensors: %d\n", this, sensor[this].blocked, blockedSensors);
                pthread_cond_wait(&doMaintenceCond[this], &sensor[this].mutex);
                printf("\t\tSensor %d: Manutencao\n", this);
    ...
    pthread_barrier_wait(&finalMaintence);
    printf("\t\tSensor %d: Manutencao\n", this);
    pthread_barrier_wait(&finalMaintenceMessage);
}
...
void * controlThread(){
    ...
    while(doFinalmaintence == 0){
        ...
        if(blockedSensors == S){
            ...
            //finish program if all the luggage was analyzed
            if(xrayedLuggageCount >= N*M) doFinalmaintence = 1;
        }
        else if(blockedSensors > 0 && xrayedLuggageCount >= N*M){ //in case only a few sensors are blocked and all the luggage was analyzed
            while(blockedSensors != S) {
                //try to force all sensros to go to maintence
                lockAllSensors();
                for (i = 0; i < S; ++i) { //force a maintence
                    sensor[i].count = Q;
                }
                unlockAllSensors();
            }
...
void * sendLuggageToSensor(void * line) {
    ...
    while (xrayedLuggageCount < N*M) { //lines must finish only when all sensors finish because they might have to reallocate luggage
        ...
        //printf("line %d, sensor %d: queue: %d, blocked: %d lock acquired, the rest:\n", this, s, sensor[s].queue, sensor[s].blocked);
        //for(i=0;i<S;i++)printf("line %d, sensor %d: queue: %d, blocked: %d lock acquired\n", this, i, sensor[i].queue, sensor[i].blocked);
        if (sensor[s].queue < 5 && sensor[s].blocked == 0) {
            sensor[s].queue++;
            pthread_mutex_lock(&reallocationBufferMutex);
            ...
            printf("Line %d: sensor[%d].queue = %d, remaining: %d, reallocationBuffer %d.\n", this, s, sensor[s].queue, remainingLuggage, reallocationBuffer);
            pthread_mutex_unlock(&reallocationBufferMutex);
        }
        pthread_mutex_unlock(&sensor[s].mutex); //unlock sensor that's still locked from getLeastBusySensor()
        //printf("line %d, sensor: %d, lock realeased\n", this, s);
...
int main(int argc, char const *argv[]) {
    ...
    pthread_barrier_init(&maintenceCompleteBarrier, NULL, S+1); //all sensors plus control thread
    pthread_barrier_init(&finalMaintence, NULL, S); //all sensors plus control thread
    pthread_barrier_init(&finalMaintenceMessage, NULL, S+1); //all sensors plus control thread
